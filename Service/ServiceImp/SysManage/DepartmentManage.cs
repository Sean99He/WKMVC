...
namespace Service.ServiceImp
{
	public class DepartmentManage : RepositoryBase<SYS_DEPARTMENT>, IDepartmentManage
	{
		...
		/// <summary>
		/// 新增或者编辑 created by Sean.He
		/// </summary>
		/// <param name="module"></param>
		/// <returns></returns>
		public bool SaveOrUpdate(SYS_DEPARTMENT module)
		{
			module.NAME = module.NAME.Trim().Replace("&nbsp;", "").Replace("|--", "");
			if (string.IsNullOrEmpty(module.ID))
			{
				var sql = "select CONVERT(nvarchar(36), max(a.ID)+1) From SYS_DEPARTMENT a";
				var id = this.SelectBySql<string>(sql)[0];
				module.ID = id;
		...
		/// <summary>
		/// 得到上级部门下拉菜单 created by Sean.He
		/// </summary>
		/// <returns></returns>
		public string GetFirstLevelDepartment()
		{
			var result = this.LoadListAll(m => m.PARENTID == "0");
			StringBuilder builder = new StringBuilder();
			builder.Append("<option value='0'>一级菜单</option>");
			if (result.Count > 0)
			{
		...
		/// <summary>
		/// 根据部门按级别排序 2016-11-28 17:00:20
		/// </summary>
		/// <param name="list"></param>
		/// <returns></returns>
		public List<SYS_DEPARTMENT> SortDepartmentList(List<SYS_DEPARTMENT> list)
		{
			var result = new List<SYS_DEPARTMENT>();
			var itemList = list.Where(m => m.PARENTID == "0");
			foreach (var item in itemList)
			{
				result.Add(item);
				result.AddRange(list.Where(m => m.PARENTID == item.ID));
			}
			return result;
		}
	}
